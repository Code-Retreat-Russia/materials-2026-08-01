...
#box(
  ...
  [
    #set text(size: 11pt)
    - На демонстрации результата мы хотим видеть, что у вас было не one-shot решение.
    - Можно и даже нужно, полностью генерировать код агентом.
...
- *Понятно только разработке*: это не дает всей ценности тестов, потому что они перестают касаться заказчиков и теряется обратная связь для них.
- *Много деталей в тестах*: обвязка сделана так, чтобы нельзя читать сценарии без погружения в то, как устроено приложение.
- *Моргают*: это проблема всех тестов высокого уровня, для этого надо изолировать систему.
...
Форматирование авторское, можете придумать свое, обязательно: 

- итоговая цена, 
- примененные спец. цены.
